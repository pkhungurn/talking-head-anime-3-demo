...
def slider_component():
    ...
    input_pose = InputPose(
        eyebrow_dropdown, eyebrow_left, eyebrow_right,
        eye_dropdown, eye_left, eye_right,
        mouth_dropdown, mouth_left, mouth_right,
        iris_small_left, iris_small_right, iris_rotation_x, iris_rotation_y,
        head_x, head_y, neck_z,
        body_y, body_z, breathing
    )
...
def pil_image_from_pytorch_image(pytorch_image):
    ...
    np_image = np.uint8(np.rint(convert_output_image_from_torch_to_numpy(output_image) * 255.0))
